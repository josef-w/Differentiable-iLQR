...
from lqr_step_explicit import LQRStep
...
class MPC(Module):
    ...
    def forward(self, x_init, cost, dx):
        ...
        x = torch.cat(best['x'], dim=1).detach().requires_grad_(True)
        u = torch.cat(best['u'], dim=1).detach().requires_grad_(True)
        ...
        l_f=torch.ones(F.shape)
        #torch.autograd.grad(F, x, grad_outputs=l_f, retain_graph=True)[0]
        if max(best['full_du_norm']) > self.eps:
            if_converge=False
        else:
            if_converge = True
        # print('If converged?',if_converge)
        x, u= self.solve_lqr_subproblem(
            x_init, C, c, F, f, cost, dx, x, u, no_op_forward=True,theta=dx.params,if_converge=if_converge)

        # def linear_dx(theta):
        #     dx.params=theta
        #     return self.linearize_dynamics(x,u,dx,diff=True)
        # J1 = torch.autograd.functional.jacobian(lambda t:linear_dx(t)[0], dx.params)
        # J2 = torch.autograd.functional.jacobian(lambda t: linear_dx(t)[1], dx.params)
        # print(J1.shape,J2.shape)
        # grad_D, grad_d,D_grad_x, D_grad_u,D,d_grad_X,d_grad_U=dx.grad_input(x,u)
        # print(torch.sum(J2-grad_d))
        # future_j1 = torch.jit.fork(
        #     lambda: torch.autograd.functional.jacobian(lambda t: linear_dx(t)[0], dx.params))
        # future_j2 = torch.jit.fork(
        #     lambda: torch.autograd.functional.jacobian(lambda t: linear_dx(t)[1], dx.params))
        #
        # # 等待异步任务完成
        # J1 = torch.jit.wait(future_j1)
        # J2 = torch.jit.wait(future_j2)
    ...
    def solve_lqr_subproblem(self, x_init, C, c, F, f, cost, dynamics, x, u,
                             no_op_forward=False,theta=None,if_converge=False):
        if self.slew_rate_penalty is None or isinstance(cost, Module):
            ...
            return _lqr(x_init, C, c, F, f if f is not None else e, theta)
        else:
...
""")
    ...
    def linearize_dynamics(self, x, u, dynamics, diff):
        ...
        if self.grad_method == GradMethods.ANALYTIC:
            ...
            _x = x[:-1].contiguous().view(-1, self.n_state)
            ...
            D=dynamics.get_linear_dyn(_x,_u)
            d=_new_x-torch.einsum('bnm,bm->bn',D,torch.cat((_x, _u), dim=-1))
            #f = _new_x - util.bmv(R, _x) - util.bmv(S, _u)
            d = d.view(self.T-1, n_batch, self.n_state)
            ...
            # F = torch.cat((R, S), 3)
            D = D.contiguous().view(self.T - 1, n_batch, self.n_state, self.n_state+self.n_ctrl)
            F,f=D,d
            if not diff:
        ...
        Re-run with GradMethods.ANALYTIC to continue.
            ...
            if diff:
                pass
                # l_f = torch.ones(F.shape)
                # torch.autograd.grad(F, x_init, grad_outputs=l_f, retain_graph=True)[0]
            if not diff:
                F, f = list(map(Variable, [F, f]))
            return F, f
